...
object OverviewPage : PageHandler {
    ...
    override fun draw(ctx: DrawContext, x: Float, y: Float, w: Float, h: Float, mouseX: Double, mouseY: Double) {
        ...
        activePet?.heldItem.let { heldId ->
            ...
            val petLineY    = statsTop + (lines.size - 1) * lineSpacing + (lineSpacing - PET_SIZE) / 2f
            val textEndX    = x + PADDING + ctx.formattedTextWidth(lines.last(), size)
            val icon        = heldId?.let { RepoItemsAPI.getItem(it) }
                ?: ItemStack(Items.BARRIER)
            ctx.text("(", textEndX + 2f, petLineY + (PET_SIZE - size) / 2f, size, Color(170, 170, 170))
            ctx.item(icon, textEndX + ctx.textWidth("(", size) + 4f, petLineY, PET_SIZE)
            ctx.text(")", textEndX + ctx.textWidth("(", size) + 4f + PET_SIZE + 2f, petLineY + (PET_SIZE - size) / 2f, size, Color(170, 170, 170))
        }
    }
